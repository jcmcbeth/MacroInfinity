...
namespace Infantry.Cfs
{
    ...
    {	
        ...
        /// <summary>
        /// Gets if the sprite contains transparent pixels.
        /// </summary>
        ...
        /// <summary>
        /// Loads the CFS file into memory.
        /// </summary>
        public void Load()
        {
            byte[] buffer;
            ...
            {                
                ...
                int regularSize = (_width * _height) * _frameCount;
                if (regularSize != _dataSize)
                    _transparent = true;
                else
                    _transparent = false;
                ...
                ReadFrameInfo(out _frames[index], buffer, offset);
                offset += 12;
                               
                // This calculates the length of the previous frame by subtracting
                // the offset of the previous frame from the offset of the current frame.
                while (index < (_frames.Length - 1))
                {
                    index++;
                    ...
                    // Then calculate the length of the previous frame
                    _frames[index - 1].Length = _frames[index].DataOffset - _frames[index - 1].DataOffset;                                       
                }                

                // Now find the length of the last frame
                _frames[index].Length = _dataSize - _frames[index].DataOffset;

                if (_userDataLength > 0)
                {
                    _userData = new byte[_userDataLength];
                    Buffer.BlockCopy(buffer, offset, _userData, 0, _userData.Length);
                    offset += _userData.Length;
                }

                for (int i = 0; i < _frames.Length; i++)             
                    ReadFrameData(ref _frames[i], buffer, offset, _transparent);                
            }
            else
            {
        ...
        /// </summary>
        /// <remarks>It increments the value of the offset to a position after the frame info.</remarks>
        ...
        private static void ReadFrameInfo(out Frame frame, byte[] buffer, int offset)
        {
            frame = new Frame();
        ...
        /// <summary>
        /// Reads the bitmap data for the frame from a buffer.
        /// </summary>
        /// <param name="frame">The frame to read.</param>
        /// <param name="buffer">The buffer to read the frame from.</param>
        /// <param name="offset">The index where the frame data for the file begins.</param>
        /// <param name="transparent">If the frame is transparent. </param>
        private static void ReadFrameData(ref Frame frame, byte[] buffer, int offset, bool transparent)
        {
            int dataLength;
            int newOffset;
            ...
            newOffset = offset + frame.DataOffset;
            ...
            if (transparent)
            {
                // Read in the extra info
                frame.Strides = new byte[height];
                Buffer.BlockCopy(buffer, newOffset, frame.Strides, 0, frame.Strides.Length);
                newOffset += frame.Strides.Length;

                dataLength = 0;
                for (int i = 0; i < frame.Strides.Length; i++)
                    dataLength += frame.Strides[i];
            }
            else
            {
                frame.Strides = new byte[0];
                dataLength = width * height;
            }

            // Read in the data after the extra info            
            frame.ScanLines = new byte[dataLength];
            Buffer.BlockCopy(buffer, newOffset, frame.ScanLines, 0, frame.ScanLines.Length);             
        }
